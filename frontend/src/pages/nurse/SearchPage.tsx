                                ...
                                <>
                                    ...
                                    <div style={{ background: 'var(--surface-container-lowest)', padding: '2rem', borderRadius: 'var(--radius-xl)', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', borderTop: `4px solid ${triageLevel === 'Red' ? '#dc2626' : triageLevel === 'Yellow' ? '#f59e0b' : '#10b981'}` }}>
                                        ...
                                        <div style={{ marginBottom: '1.5rem' }}>
                                            <label style={{ display: 'block', fontSize: '0.9rem', fontWeight: 600, color: 'var(--on-surface)', marginBottom: '0.5rem' }}>Triage Urgency Level (Leave unselected for Normal)</label>
                                            <div style={{ display: 'flex', gap: '1rem' }}>
                                                <button onClick={() => setTriageLevel(triageLevel === 'Red' ? 'Green' : 'Red')} style={{ flex: 1, padding: '0.75rem', borderRadius: 'var(--radius-md)', border: `2px solid ${triageLevel === 'Red' ? '#dc2626' : 'var(--outline-variant)'}`, background: triageLevel === 'Red' ? '#fef2f2' : 'var(--surface)', color: triageLevel === 'Red' ? '#dc2626' : 'var(--on-surface-variant)', fontWeight: 600, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', transition: 'all 0.2s ease', opacity: triageLevel === 'Yellow' ? 0.5 : 1 }}>
                                                    {triageLevel === 'Red' ? '🔴 EMERGENCY SELECTED' : '🔴 Mark as Emergency'}
                                                </button>
                                                <button onClick={() => setTriageLevel(triageLevel === 'Yellow' ? 'Green' : 'Yellow')} style={{ flex: 1, padding: '0.75rem', borderRadius: 'var(--radius-md)', border: `2px solid ${triageLevel === 'Yellow' ? '#f59e0b' : 'var(--outline-variant)'}`, background: triageLevel === 'Yellow' ? '#fffbeb' : 'var(--surface)', color: triageLevel === 'Yellow' ? '#b45309' : 'var(--on-surface-variant)', fontWeight: 600, display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem', transition: 'all 0.2s ease', opacity: triageLevel === 'Red' ? 0.5 : 1 }}>
                                                    {triageLevel === 'Yellow' ? '🟡 URGENT SELECTED' : '🟡 Mark as Urgent'}
                                                </button>
                                            </div>
                                        </div>
